...
use fatrw::read_file as fatrw_read_file;
use fatrw::write_file as fatrw_write_file;

use anyhow::{Context, Result};

pub fn read_file(path: &Path) -> Result<String> {
    Ok(String::from_utf8(fatrw_read_file(path).context(
        format!("Reading {:?} failed", path.to_path_buf()),
    )?)?)
}

pub fn write_file(path: &Path, contents: &str, mode: Option<u32>) -> Result<()> {
    fatrw_write_file(path, contents.as_bytes(), mode)
